...
void test_eularparse_push_pop()
{
	...
	test(!strcmp(e->stack.data[e->stack.sptr - 1].name, "foo/bar"));
	struct eularparse_stack stack = eularparse_pop(e);
	test(!strcmp(e->stack.data[e->stack.sptr - 1].name, "foo"));
	test(!strcmp(stack.name, "foo"));
	eularparse_push(e, "foobar", 0);
	test(!strcmp(e->stack.data[e->stack.sptr - 1].name, "foo/foobar"));
	eularparse_push(e, "bar", 0);
	test(!strcmp(e->stack.data[e->stack.sptr - 1].name, "foo/foobar/bar"));
	eularparse_push(e, "hello", 1);
	test(!strcmp(e->stack.data[e->stack.sptr - 1].name, "hello"));
	eularparse_push(e, "world", 0);
	test(!strcmp(e->stack.data[e->stack.sptr - 1].name, "hello/world"));
	...
	test(!strcmp(e->stack.data[e->stack.sptr - 1].name, "hello///world///0"));
	eularparse_increment(e);
	test(!strcmp(e->stack.data[e->stack.sptr - 1].name, "hello///world///1"));
	eularparse_push(e, "we meet again!", 0);
	test(!strcmp(e->stack.data[e->stack.sptr - 1].name, "hello///world///1///we meet again!"));
	eularparse_increment(e);
	test(!strcmp(e->stack.data[e->stack.sptr - 1].name, "hello///world///1///we meet again!///0"));
	stack = eularparse_pop(e);
	test(!strcmp(e->stack.data[e->stack.sptr - 1].name, "hello///world///1"));
	destroy_eularparse(e);
}
...
void test_eularparse_mask()
{
	...
	unsigned char *field_mask = mem.xc(256, sizeof(char));
	eularparse_set_mask(
		field_mask,
		0
...
void *handle_arr_start(void *arg)
{
	...
	if (info->subtype[AT] == BEGIN) {
		...
		test(!strcmp(arrname, "obj/0/arr/0"));
	}
	return (void *)p;
}
...
void test_parsing()
{
	...
	unsigned char *number_mask = mem.xc(256, sizeof(char));
	eularparse_set_mask(
		number_mask,
		0
		| EP_MASK_NUMBER
	...
	struct demo_parse *p = mem.xm(sizeof(struct demo_parse));
	p->ep = ep;
	p->ch = teststr;
	p->pos = 0;
